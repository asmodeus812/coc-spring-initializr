...
export function matchRange(version: string, range: string): boolean {
    const strictMatchGrp: RegExpMatchArray | null = RegExp(strictRange).exec(range);
    if (strictMatchGrp) {
        return compareVersions(strictMatchGrp[1], version) <= 0 && compareVersions(strictMatchGrp[2], version) >= 0;
    }
    const horMatchGrp: RegExpMatchArray | null = RegExp(halfopenRightRange).exec(range);
    if (horMatchGrp) {
        return compareVersions(horMatchGrp[1], version) <= 0 && compareVersions(horMatchGrp[2], version) > 0;
    }
    const holMatchGrp: RegExpMatchArray | null = RegExp(halfopenLeftRange).exec(range);
    if (holMatchGrp) {
        return compareVersions(holMatchGrp[1], version) < 0 && compareVersions(holMatchGrp[2], version) >= 0;
    }
